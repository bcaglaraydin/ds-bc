...
def custom_loss3(y_pred, y_true, b1, b0, t, lam):
    return t * (1 - (1 / math.exp(np.absolute(y_pred - y_true).mean()))) + lam * np.linalg.norm(np.array([b0, b1]))


def regression_model(x, y, thresh, alpha=0.001, max_iter=1000, lam=0.5):
    b = np.random.random(2)
    for i in range(max_iter):
        y_pred = b[0] + b[1] * x

        # loss = custom_loss3(y_pred=y_pred, y_true=y, t=thresh, lam=lam, b0=b[0], b1=b[1])
        # print("loss", loss)
        e = np.expm1((y - y_pred))

        if(y_pred.mean() >= y.mean()):
            print("1")
            g_b0 = thresh * e.mean() + 2 * lam * b[0]
            g_b1 = thresh * (x * -e).mean() + 2 * lam * b[1]
        else:
            print("2")
            g_b0 = -thresh * e.mean() + 2 * lam * b[0]
            g_b1 = -thresh * (x * -e).mean() + 2 * lam * b[1]
...
#         loss.append(custom_loss3(y_pred=a, y_true=y, t=thresh, b1=0, b0=0, lam=0.5))
#
#     l = pd.DataFrame(dict(a=np.linspace(-10, 10, 100), loss=loss))
#     st.dataframe(l)
...
#             b0.append(_b0)
#             b1.append(_b1)
#             loss.append(custom_loss3(y_true=y, y_pred=(_b1 * X - _b0), b1=_b1, b0=_b0, t=thresh, lam=0.5))
#
