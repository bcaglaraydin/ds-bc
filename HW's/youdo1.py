...
def custom_loss4(y_pred, y_true, b1, b0, t, lam):
...
def regression_model(x, y, t, alpha=0.001, max_iter=10000, lam=0.5):
    ...
    for i in range(max_iter):
        ...
        iteration.append(i)
        loss.append(custom_loss4(y_pred=y_pred, y_true=y, t=t, lam=lam, b0=b[0], b1=b[1]))

        diff = y - y_pred
        g_b0 = -2 * (diff * np.exp(-(np.power(diff, 2)/t))).mean() + 2 * lam * b[0]
        g_b1 =  -2 * (x * diff * np.exp(-(np.power(diff, 2)/t))).mean() + 2 * lam * b[1]
...
def convex_check(X, y, thresh):
    ...
    for a in np.linspace(-5, 5, 50):
        loss.append(custom_loss4(y_pred=a, y_true=y, t=thresh, b1=0, b0=0, lam=0.0))
    ...
    for _b0 in np.linspace(-1, 1, 50):
        for _b1 in np.linspace(-1, 1, 50):
            b0.append(_b0)
            b1.append(_b1)
            loss.append(custom_loss4(y_true=y, y_pred=(_b1 * X - _b0), b1=_b1, b0=_b0, t=thresh, lam=0.0))
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.markdown("Function looks convex.")
...
def make_regression(X, y, t, alpha=0.001, lam=0.5):
    ...
    st.markdown(fr"mse {mean_squared_error(y, y_pred)}")
...
st.header("You Do #1")
st.header("Berdan Çağlar Aydın")
st.subheader("Dataset")
X, y = load_dataset()
st.subheader("Custom Loss Function")
st.markdown("This function gets very close to theta but doesn't touch it.")
st.markdown("https://www.desmos.com/calculator/wxunklphmt")
st.latex(r"L(\beta_0, \beta_1) = \sum_{i=1}^{N}\Theta * (1 - e^{-\frac{(y_i - (\beta_0 + \beta_1 x_i))^2}{\Theta}}) + \lambda (\beta_0^2 + \beta_1^2), (\lambda > 0), (\Theta > 0)")

thresh = st.slider("Loss Threshold", min_value=0., max_value=10., value=5.0, step=0.2)
convex_check(X, y, thresh=thresh)
